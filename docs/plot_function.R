...
plot_expression_and_amreads <- function(query_genes, title_prefix, trt_filter = c("M", "C"), time_filter = c("T1", "T2")) {
  ...
  tryCatch({
    ...
    DE_ASE_table <- read_csv("data/DE_ASE_table.csv")
    
    ##---------------------------------------------------------
    ## Step 2: Filter selected genes first
    ##---------------------------------------------------------
    ...
    norm_counts_DE <- read_csv("data/norm_counts_DE.csv") %>%
      select(-"...1") %>%
    ...
    long <- norm_counts_DE %>%
      pivot_longer(cols = matches("_M_|_C_"), names_to = "Sample", values_to = "norm_counts") %>%
      mutate(label = Sample) %>%
      left_join(Total_AMreads_df, by = "label") %>%
      separate(Sample, into = c("genotype", "trt", "time", "rep"), sep = "_") %>%
      mutate(
        generation = case_when(
          genotype == "Oh43xB73" ~ "F1",
          genotype %in% c("Oh43", "B73") ~ "F0"
        ),
        genotype = factor(genotype, levels = c("Oh43", "B73", "Oh43xB73")),
        generation = factor(generation, levels = c("F0", "F1"))
      ) %>%
      filter(!str_detect(genotype, "W22"))
    
    # Apply filters after data processing
    long <- long %>% 
      filter(trt %in% trt_filter, time %in% time_filter)
    
    ##---------------------------------------------------------
    ## Step 6: Summary statistics (mean ± SE)
    ##---------------------------------------------------------
    message("Calculating summary statistics...")
    long_summary <- long %>%
      group_by(genotype, time, geneID, trt, generation, Working.Symbol, ASE_sign) %>%
      summarise(
    ...
    long_summary$Working.Symbol <- factor(long_summary$Working.Symbol, levels = ordered_genes$Working.Symbol)
    long$Working.Symbol <- factor(long$Working.Symbol, levels = ordered_genes$Working.Symbol)
    ...
    p1 <- ggplot() +
      geom_point(data = long, aes(x = trt, y = norm_counts, color = genotype, group = interaction(geneID, genotype)), alpha = 0.1) +
      geom_line(data = long_summary, aes(x = trt, y = mean_norm_counts, color = genotype, group = interaction(geneID, genotype)), linewidth = 1) +
      geom_point(data = long_summary, aes(x = trt, y = mean_norm_counts, color = genotype), size = 3) +
      geom_errorbar(data = long_summary, aes(x = trt, ymin = mean_norm_counts - se_norm_counts, ymax = mean_norm_counts + se_norm_counts, color = genotype), width = 0.1) +
    ...
    p2 <- ggplot() +
      geom_point(data = long, aes(x = norm_counts, y = Total_AMreads, color = genotype), alpha = 0.1) +
      geom_point(data = long_summary, aes(x = mean_norm_counts, y = mean_amreads, color = genotype), size = 3) +
      geom_errorbar(data = long_summary, aes(x = mean_norm_counts, ymin = mean_amreads - se_amreads, ymax = mean_amreads + se_amreads, color = genotype), width = 0.1) +
      geom_errorbarh(data = long_summary, aes(y = mean_amreads, xmin = mean_norm_counts - se_norm_counts, xmax = mean_norm_counts + se_norm_counts, color = genotype), height = 0.1) +
...
plot_ase_expression <- function(query_genes, title_prefix, trt_filter = c("M", "C"), time_filter = c("T1", "T2")) {
  ...
  tryCatch({
    ...
    norm_counts_ASE <- read_csv("data/norm_counts_ASE.csv") %>%
      select(-"...1") %>%
      filter(geneID %in% selected_genes$geneID) %>%
      right_join(selected_genes, by = "geneID")
    ...
    ##---------------------------------------------------------
    ## Step 4: Reshape data and annotate metadata columns
    ##---------------------------------------------------------
    message("Processing data for plotting...")
    long_ASE <- norm_counts_ASE %>%
      ...
      left_join(Total_AMreads_df, by = "label") %>%
      separate(Sample, into = c("delete", "genotype", "trt", "time", "rep", "allele"), sep = "_") %>%
      mutate(
        generation = case_when(
          genotype == "Oh43xB73" ~ "F1",
          genotype %in% c("Oh43", "B73") ~ "F0"
        ...
        generation = factor(generation, levels = c("F0", "F1")),
        genotype_1 = factor(genotype_1, levels = c("Oh43", "B73", "F1_Oh43allele", "F1_B73allele"))
      ) %>%
      filter(!str_detect(genotype, "W22"))
    ...
    ##---------------------------------------------------------
    ## Step 5: Summary statistics (mean ± SE)
    ##---------------------------------------------------------
    message("Calculating summary statistics...")
    long_ASE_summary <- long_ASE %>%
      group_by(allele, time, geneID, trt, generation, Working.Symbol, ASE_sign) %>%
      summarise(
        mean_norm_counts = mean(norm_counts, na.rm = TRUE),
        se_norm_counts = sd(norm_counts, na.rm = TRUE) / sqrt(n()),
        .groups = "drop"
    ...
    long_ASE_summary$Working.Symbol <- factor(long_ASE_summary$Working.Symbol, levels = ordered_genes$Working.Symbol)
    long_ASE$Working.Symbol <- factor(long_ASE$Working.Symbol, levels = ordered_genes$Working.Symbol)
    ...
    p <- ggplot() +
      geom_point(data = long_ASE, aes(x = generation, y = norm_counts, color = allele, group = interaction(geneID, allele)), alpha = 0.1) +
      geom_line(data = long_ASE_summary, aes(x = generation, y = mean_norm_counts, color = allele, group = interaction(geneID, allele)), linewidth = 1) +
      geom_point(data = long_ASE_summary, aes(x = generation, y = mean_norm_counts, color = allele), size = 3) +
      geom_errorbar(data = long_ASE_summary, aes(x = generation, ymin = mean_norm_counts - se_norm_counts, ymax = mean_norm_counts + se_norm_counts, color = allele), width = 0.1) +
